...
interface AssistantProps {
  ...
  workspaceId?: string;
  selectedDocumentIds?: string[]; // <--- Added: Selected sources from SourceDrawer
  isLoading?: boolean;
}
...
export function AIAssistant({
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Build payload for NestJS backend
      const payload: Record<string, any> = {
        question: prompt,
        workspaceId: workspaceId || "8f11ccc8-308b-43a1-a8ad-2d7f727176df",
      };

      // Only pass documentIds if user checked active items in SourceDrawer
      ...
      if (res.ok && data.success) {
        ...
        setMatchedCount(data.chunksRetrieved || 0);

        if (onResponse) {
          onResponse(data.answer, data.chunksRetrieved);
        }
      } else {
